...
WHERE P.Deleted = 0
	...
	AND P.LastModifiedDate >= CAST('2020-04-21' AS DATE)
	AND cast(P.LastModifiedDate as date) < CAST('2021-09-02' AS DATE)

drop table if exists  #TempsN
select *
INTO #TempsN
from [Dmart].[dbo].[OsagoPolises] AS P
WHERE 
	P.LastModifiedDate >= CAST('2020-04-21' AS DATE)
	AND cast(P.LastModifiedDate as date) < CAST('2021-09-02' AS DATE)

select T.*
from #Temps AS T
left join #TempsN AS N on N.HashValue = T.HashValue
where T.LastModifiedDate != ISNULL(N.LastModifiedDate, getdate())
	and T.LastModifiedDate >= CAST('2021-01-01' AS DATE)


/****** Script for SelectTopNRows command from SSMS  ******/
SELECT *
FROM [Dmart].[dbo].[OsagoPolises] 
where CAST(LastModifiedDate as date) = cast('2021-09-01 16:48:37.610' as date)
and isUploaded is null
--for system_time all
where ProductId in (20720821, 20732682)


select *
from OrantaSch.log.Products as P
where P.PolisNumber in ('АТ-9629799', 'АТ-0891589')

select *
from DmartTest.Osago.Products as P
where P.PolisNumber in ('АР-9629799', 'АТ-0891589')
...
where T.LastModifiedDate != ISNULL(N.LastModifiedDate, getdate())
	and T.LastModifiedDate >= CAST('2021-01-01' AS DATE)
	and T.[d_end] >= DATEADD(MM, -2, GETDATE())
) AS [source]
